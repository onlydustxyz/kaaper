...
suite("function-comment: view: balanceOf", () => {
    test("should've started on lineNumber 6", () => {
        ...
        assert.equal("# Explicit args:", functionCommentLine.trim(), `check lineNumber ${lineNumber}`);
        ...
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope lineNumber ${lineNumber}`);
        const functionCommentParsing = explicitArgsParser.parseCommentLine(functionCommentLine);
    ...
    test("should've found account(felt)", () => {
        ...
        const lineNumber = 7;
        const functionCommentLine = functionCommentScope.text[lineNumber];
        assert.equal("#   account(felt): account to query balance for", functionCommentLine.trim(), `check lineNumber ${lineNumber}`);
        ...
        assert.deepEqual(targetLineParsing, resultLineParsing, `failed to get resultLineParsing lineNumber ${lineNumber}`);
    ...
    test("should've ended on lineNumber 8", () => {
        ...
        const lineNumber = 8;
        const functionCommentLine = functionCommentScope.text[lineNumber];
        assert.equal("# Returns:", functionCommentLine.trim(), `check lineNumber ${lineNumber}`);
        assert.notEqual(lineNumber, explicitArgsParser.startLine);
        const isEndScope = explicitArgsParser.isEndScope(functionCommentLine);
        assert.equal(true, isEndScope, `failed to get end scope lineNumber ${lineNumber}`);
        explicitArgsParser.setEndScope(functionCommentLine);
        assert.equal(false, explicitArgsParser.runningScope, `failed to get running scope lineNumber ${lineNumber}`);
        const functionCommentParsing = explicitArgsParser.parseCommentLine(functionCommentLine);
        assert.equal(null, functionCommentParsing, `failed to get functionCommentParsing line ${lineNumber}`);
    });
    test("should've get the whole scope", () => {
        ...
        const functionCommentText = functionCommentScope.text.join("");
        const explicitArgsParser = new explicit_args_1.default(functionCommentText);
        const functionCommentParsing = explicitArgsParser.parseCommentLines(functionCommentScope.text);
        const targetLineParsing = [
            {
                name: "account",
                type: "felt",
...
suite("function-comment: view: allowance", () => {
    ...
    test("should've found owner(felt)", () => {
        ...
        const lineNumber = 7;
        const functionCommentLine = functionCommentScope.text[lineNumber];
        assert.equal("#   owner(felt): the address of owner of the tokens", functionCommentLine.trim(), `check lineNumber ${lineNumber}`);
        assert.notEqual(functionCommentLine, explicitArgsParser.startLine);
        const isEndScope = explicitArgsParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope lineNumber ${lineNumber}`);
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope lineNumber ${lineNumber}`);
        ...
        assert.equal(functionCommentReference, wholeFileReference);
        assert.equal("owner(felt): the address of owner of the tokens", functionCommentReference);
    });
    test("should've found spender(felt)", () => {
        ...
        const lineNumber = 8;
        const functionCommentLine = functionCommentScope.text[lineNumber];
        assert.equal("#   spender(felt): the address of spender (delegated account) of the tokens", functionCommentLine.trim(), `check lineNumber ${lineNumber}`);
        assert.notEqual(functionCommentLine, explicitArgsParser.startLine);
        const isEndScope = explicitArgsParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope lineNumber ${lineNumber}`);
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope lineNumber ${lineNumber}`);
        ...
        assert.equal(functionCommentReference, wholeFileReference);
        assert.equal("spender(felt): the address of spender (delegated account) of the tokens", functionCommentReference);
    });
