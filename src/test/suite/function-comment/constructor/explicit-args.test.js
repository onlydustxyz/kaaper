...
const fs = require("fs");
const explicit_args_1 = require("../../../../lib/parser/function-comment/explicit-args");
...
suite("function-comment: constructor: explicit-args", () => {
    test("parse line 6", () => {
        ...
        const functionCommentScope = CairoParser_1.default.parseCommentLines(functionScopes[0]).text;
        const functionCommentText = functionCommentScope.join("");
        const explicitArgsParser = new explicit_args_1.default(functionCommentText);
        const line = 6;
        assert.equal("# Explicit args:", functionCommentScope[line].trim(), `check line ${line}`);
        ...
        const resultLineParsing = explicitArgsParser.parseCommentLine(functionCommentScope[line]);
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope line ${line}`);
        assert.equal(false, explicitArgsParser.isEndScope(functionCommentScope[line]), `failed to get end scope line ${line}`);
    ...
    test("parse line 7", () => {
        ...
        assert.equal("#   name(felt): name of the token", functionCommentLine.trim(), `check line ${line}`);
        ...
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope line ${line}`);
        const resultLineParsing = explicitArgsParser.parseCommentLine(functionCommentLine);
    ...
    test("parse line 8", () => {
        ...
        const line = 8;
        const functionCommentLine = functionCommentScope.text[line];
        assert.equal("#   symbol(felt): symbol of the token", functionCommentLine.trim(), `check line ${line}`);
        assert.notEqual(functionCommentLine, explicitArgsParser.startLine);
        const isEndScope = explicitArgsParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope line ${line}`);
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope line ${line}`);
    ...
    test("parse line 9", () => {
        ...
        const line = 9;
        const functionCommentLine = functionCommentScope.text[line];
        assert.equal("#   decimals(Uint256): floating point of the token", functionCommentLine.trim(), `check line ${line}`);
        assert.notEqual(functionCommentLine, explicitArgsParser.startLine);
        const isEndScope = explicitArgsParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope line ${line}`);
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope line ${line}`);
        ...
        assert.equal(functionCommentReference, wholeFileReference);
        assert.equal("decimals(Uint256): floating point of the token", functionCommentReference);
    });
    test("parse line 10", () => {
        const pathFile = path.resolve(__dirname, "../../../../../testContracts/ERC20Compliant/ERC20.cairo");
        const text = fs.readFileSync(pathFile, "utf8");
        ...
        const line = 10;
        const functionCommentLine = functionCommentScope.text[line];
        assert.equal("#   initial_supply(Uint256): amount of initial supply of the token", functionCommentLine.trim(), `check line ${line}`);
        assert.notEqual(functionCommentLine, explicitArgsParser.startLine);
        const isEndScope = explicitArgsParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope line ${line}`);
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope line ${line}`);
        ...
        assert.equal(functionCommentReference, wholeFileReference);
        assert.equal("initial_supply(Uint256): amount of initial supply of the token", functionCommentReference);
    });
    test("parse line 11", () => {
        const pathFile = path.resolve(__dirname, "../../../../../testContracts/ERC20Compliant/ERC20.cairo");
        const text = fs.readFileSync(pathFile, "utf8");
        ...
        const line = 11;
        const functionCommentLine = functionCommentScope.text[line];
        assert.equal("#   recipient(felt): the address of recipient of the initial supply", functionCommentLine.trim(), `check line ${line}`);
        assert.notEqual(functionCommentLine, explicitArgsParser.startLine);
        const isEndScope = explicitArgsParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope line ${line}`);
        assert.equal(true, explicitArgsParser.runningScope, `failed to get running scope line ${line}`);
        ...
        assert.equal(functionCommentReference, wholeFileReference);
        assert.equal("recipient(felt): the address of recipient of the initial supply", functionCommentReference);
    });
    test("parse line 12", () => {
        const pathFile = path.resolve(__dirname, "../../../../../testContracts/ERC20Compliant/ERC20.cairo");
        const text = fs.readFileSync(pathFile, "utf8");
        ...
        assert.equal(true, explicitArgsParser.isEndScope(functionCommentScope[line]), `failed to get end scope line ${line}`);
    ...
    test("parse whole scope", () => {
        ...
        const resultLineParsing = explicitArgsParser.parseCommentLines(functionCommentScope);
        assert.deepEqual(targetLineParsing, resultLineParsing, `failed to get resultLineParsing on whole scope`);
    });
});
//# sourceMappingURL=explicit-args.test.js.map