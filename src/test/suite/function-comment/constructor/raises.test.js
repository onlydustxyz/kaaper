...
const fs = require("fs");
const raises_1 = require("../../../../lib/parser/function-comment/raises");
...
suite("function-comment: constructor: raises", () => {
    test("parse line 14", () => {
        ...
        const raisesParser = new raises_1.default(functionCommentText);
        const line = 14;
        ...
        const resultLineParsing = raisesParser.parseCommentLine(functionCommentLine);
        assert.equal(true, raisesParser.runningScope, `failed to get running scope line ${line}`);
        assert.equal(false, raisesParser.isEndScope(functionCommentLine), `failed to get end scope line ${line}`);
    ...
    test("parse line 15", () => {
        ...
        const line = 15;
        const functionCommentLine = functionCommentScope.text[line];
        assert.equal("#   decimals: decimals exceed 2^8", functionCommentLine.trim(), `check line ${line}`);
    ...
    test("parse line 16", () => {
        ...
        const line = 16;
        const functionCommentLine = functionCommentScope.text[line];
        assert.equal("#   recipient: cannot mint to the zero address", functionCommentLine.trim(), `check line ${line}`);
        assert.notEqual(functionCommentLine, raisesParser.startLine);
        const isEndScope = raisesParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope line ${line}`);
        assert.equal(true, raisesParser.runningScope, `failed to get running scope line ${line}`);
    ...
    test("parse line 17", () => {
        ...
        const line = 17;
        const functionCommentLine = functionCommentScope.text[line];
        assert.equal("#   initial_supply: not valid Uint256", functionCommentLine.trim(), `check line ${line}`);
        assert.notEqual(functionCommentLine, raisesParser.startLine);
        const isEndScope = raisesParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope line ${line}`);
        assert.equal(true, raisesParser.runningScope, `failed to get running scope line ${line}`);
    ...
    test("parse line 18", () => {
        ...
        const line = 18;
        const functionCommentLine = functionCommentScope.text[line];
        assert.equal("#   initial_supply: mint overflow", functionCommentLine.trim(), `check line ${line}`);
        assert.notEqual(functionCommentLine, raisesParser.startLine);
        const isEndScope = raisesParser.isEndScope(functionCommentLine);
        assert.equal(false, isEndScope, `failed to get end scope line ${line}`);
        assert.equal(true, raisesParser.runningScope, `failed to get running scope line ${line}`);
    ...
    test("parse whole scope", () => {
        ...
        const resultLineParsing = raisesParser.parseCommentLines(functionCommentScope.text);
        assert.deepEqual(targetLineParsing, resultLineParsing, `failed to get resultLineParsing on whole scope`);
    });
});
//# sourceMappingURL=raises.test.js.map