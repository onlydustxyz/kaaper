...
suite("scope: constructor test", () => {
    ...
    test("get implicit args comments", () => {
        ...
        if (comments){
            ...
            const isStartNode = constructorParser.isStartNode(comments[6], "Implicit args");
            const isEndNode = constructorParser.isEndNode(comments[6], "Implicit args");
            assert.equal(false, isStartNode, 'failed to get comments');
            assert.equal(true, isEndNode, 'failed to get comments');
        }
    })
    ...
    test("get explicit args comments", () => {
        ...
        if (comments){
            assert.equal("#   symbol(felt): the address of the ERC20 recipient", comments[8].trim(), 'failed to get comments');
            const isStartNode = constructorParser.isStartNode(comments[8], "Explicit args");
            const isEndNode = constructorParser.isEndNode(comments[8], "Explicit args");
            assert.equal(false, isStartNode, 'failed to get comments');
            assert.equal(false, isEndNode, 'failed to get comments');
        }
        ...
        if (comments){
            assert.equal("#   recipient(felt): amount of ERC20 transfer", comments[11].trim(), 'failed to get comments');
            const isStartNode = constructorParser.isStartNode(comments[11], "Explicit args");
            const isEndNode = constructorParser.isEndNode(comments[11], "Explicit args");
            assert.equal(false, isStartNode, 'failed to get comments');
            assert.equal(false, isEndNode, 'failed to get comments');
        }
        ...
        if (comments){
            ...
            const isStartNode = constructorParser.isStartNode(comments[12], "Explicit args");
            const isEndNode = constructorParser.isEndNode(comments[12], "Explicit args");
            assert.equal(false, isStartNode, 'failed to get comments');
            assert.equal(true, isEndNode, 'failed to get comments');
        }
