...
test("parse whole scope", () => {
  ...
  const functionSignatureExplicitArgsResult =
  ...
  assert.deepEqual(
    functionSignatureImplicitArgsTarget,
    functionSignatureImplicitArgsResult,
    "failed to get implicit args"
  );
  assert.deepEqual(
    functionSignatureExplicitArgsTarget,
    functionSignatureExplicitArgsResult,
  ...
  const functionCommentDescParserResult =
    functionCommentDescParser.parseCommentLines(commentLines!);
  const functionCommentImplicitArgsResult =
    functionCommentImplicitArgsParser.parseCommentLines(commentLines!);
  const functionCommentExplicitArgsResult =
    functionCommentExplicitArgsParser.parseCommentLines(commentLines!);
  const functionCommentReturnsResult =
    functionCommentReturnsParser.parseCommentLines(commentLines!);
  const functionCommentRaisesResult =
    functionCommentRaisesParser.parseCommentLines(commentLines!);

  const functionCommentDescParserTarget = [
  ...
  assert.deepEqual(
    functionCommentDescParserTarget,
    functionCommentDescParserResult,
    "failed to get desc"
  );
  assert.deepEqual(
    functionCommentImplicitArgsParserTarget,
    functionCommentImplicitArgsResult,
    "failed to get implicit args"
  );
  assert.deepEqual(
    functionCommentExplicitArgsParserTarget,
    functionCommentExplicitArgsResult,
    "failed to get explicit args"
  );
  assert.deepEqual(
    functionCommentReturnsParserTarget,
    functionCommentReturnsResult,
    "failed to get returns"
  );
  assert.deepEqual(
    functionCommentRaisesParserTarget,
    functionCommentRaisesResult,
